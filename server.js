  ...
  io.in(socket.room)
    ...
    .then((users) => {
      ...
      users.forEach((s) => {
        ...
        if (roomData[socket.room].currentDrawer.id !== s.id) {
          ...
          results.push({
            id: s.clientID,
            username: s.username,
            score: roomData[socket.room].score[s.clientID] ?? 0,
          });
        }
      });
      ...
      roomData[socket.room].score[
        roomData[socket.room].currentDrawer.clientID
      ] += Math.floor(totalDrawerScore / 4);
      ...
      roomData[socket.room].intermissionTimer = setTimeout(() => {
        ...
        if (
          roomData[socket.room].rosterIndex >=
          roomData[socket.room].roster.length
        ) {
...
function startOfRound(socket) {
  ...
  roomData[socket.room].currentDrawer =
    roomData[socket.room].roster[roomData[socket.room].rosterIndex];
  ...
  io.to(roomData[socket.room].currentDrawer.id).emit("wordToDraw", {
    message: guessword,
  });
  io.in(socket.room).emit("whoIsDrawing", {
    message: {
      id: roomData[socket.room].currentDrawer.clientID,
      username: roomData[socket.room].currentDrawer.username,
    },
  ...
  roomData[socket.room].roundStartTimer = setTimeout(() => {
    ...
    io.to(socket.room)
      .except(roomData[socket.room].currentDrawer.id)
      .emit("wordToGuess", { message: wordAsBlanks(guessword) });
    // Everyone gets match time sent
    io.in(socket.room).emit("matchTime", {
      message: roomData[socket.room].roundTimeMax,
    });

    //console.log("Roster is:",roomData[socket.room].roster);
    console.log(
      roomData[socket.room].currentDrawer.id,
    ...
    roomData[socket.room].timer = setInterval(() => {
      ...
      if (roomData[socket.room].roundTimeCurrent < 0) {
        ...
        io.in(socket.room).emit("roundOver", {
          message: {
            reason: "timeout",
            word: roomData[socket.room].currentWord,
          },
        });
        endOfRound(socket);
...
io.on("connection", (socket) => {
  ...
  socket.on("join", async (room, username) => {
    const temp = username.trim().slice(0, 16);
    if (!temp || temp === "") {
    ...
    if (room !== "") {
      if (room in roomData) {
        ...
        socket.join(room);
        socket.emit("validRoom", { message: room });
        socket.room = room;
        ...
        if (roomData[room].roundInProgress) {
          ...
          socket.emit("matchTime", {
            message: roomData[socket.room].roundTimeCurrent,
          });
      ...
      room = nanoid();
      console.log(
        "Room unspecified! Creating new room with the following ID:",
        room
      );
      socket.join(room);
      socket.emit("validRoom", { message: room });
    ...
    socket.emit("gameData", {
      message: {
        host: roomData[socket.room].host.clientID,
        gameStarted: roomData[socket.room].gameStarted,
      },
    });

    // Notify lobby a new user has joined
    io.in(socket.room).emit("message", {
      message: { sender: "", content: `${socket.username} has joined!` },
    });
  ...
  socket.on("disconnect", async () => {
    ...
    io.in(socket.room).emit("message", {
      message: { sender: "", content: `${socket.username} has left` },
    });
    io.in(socket.room).emit("userLeft");

    if (
      roomData[socket.room] != undefined &&
      roomData[socket.room].roster != undefined
    ) {
      const newRoster = [];
    ...
    getUsersInRoom(socket.room).then((users) => {
      ...
      if (users.length >= 1) {
        ...
        if (
          roomData[socket.room].roundInProgress &&
  ...
  socket.on("message", (message) => {
    ...
    if (!roomData[socket.room].roundInProgress) {
      io.to(socket.room).emit("message", {
        message: { sender: socket.username, content: validateMsg },
      });
    } else {
      if (
        roomData[socket.room].currentDrawer.id !== socket.id &&
        ...
        if (sanitizedGuess === roomData[socket.room].currentWord) {
          ...
          const score = Math.ceil(
            (roomData[socket.room].roundTimeCurrent /
              roomData[socket.room].roundTimeMax) *
              1000
          );
          ...
          console.log(
            "Users who have guessed correctly:",
            roomData[socket.room].guessedCorrectly
          );
          console.log("User's scores:", roomData[socket.room].score);

          // Reveal the word to the guesser now that they got it
          io.to(socket.id).emit("wordToGuess", {
            message: roomData[socket.room].currentWord,
          });
            ...
            .then((users) => {
              ...
              users.forEach((s) => {
                if (s.id !== roomData[socket.room].currentDrawer.id) {
                  if (
                    roomData[socket.room].guessedCorrectly[s.clientID] ===
                      undefined ||
  ...
  socket.on("draw", (data) => {
    ...
    if (
      ...
      io.to(socket.room)
        .except(socket.id)
        .emit("draw", { message: { x: data.x, y: data.y, type: data.type } });
    }
  });
  ...
  socket.on("canvasBrushSize", (data) => {
    ...
    if (
      ...
      io.to(socket.room)
        .except(socket.id)
        .emit("canvasBrushSize", { message: data });
    }
  });
  ...
  socket.on("replyCanvas", async (data) => {
    roomData[socket.room].newUsers.forEach((s) => {
      ...
      io.to(s.id).emit("whoIsDrawing", {
        message: {
          id: roomData[socket.room].currentDrawer.clientID,
          username: roomData[socket.room].currentDrawer.username,
        },
      });
      io.to(s.id).emit("wordToGuess", {
        message: wordAsBlanks(roomData[socket.room].currentWord),
      });

      Object.entries(roomData[socket.room].guessedCorrectly).forEach(
        ([key]) => {
      ...
      Object.entries(roomData[socket.room].score).forEach(([key, value]) => {
        io.to(s.id).emit("userScore", {
          message: { userID: key, score: value },
        });
      });
    });
  ...
  socket.on("matchStart", async () => {
    ...
    if (
      roomData[socket.room].host.id === socket.id &&
      roomData[socket.room].gameStarted === false
    ) {
  ...
  socket.on("kick", async (data) => {
    // Make sure the host can only call this command
    // Can't kick ourselves :P

    if (
      roomData[socket.room].host.id === socket.id &&
      data != roomData[socket.room].host.clientID
    ) {
      getUsersInRoom(socket.room).then((users) => {
        if (users != undefined) {
          users.forEach((s) => {
            if (s.userID === data) {
              io.in(socket.room).emit("message", {
                message: {
                  sender: "",
                  content: `${s.username} has been kicked`,
                },
              });
              console.log(`${s.socketID} has been kicked`)
              io.to(s.socketID).emit("error", { message: "You have been kicked" });
              io.to(s.socketID).emit("forceDisconnect");
            }
          });
        }
      });
